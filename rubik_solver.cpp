...
char	search_mode;
char	history_index;
char	history_moves[48];
char	history_rotpt[48];
...
void rotate( char cur_phase ) {
	int i;
	if (cur_phase < 4) {
		for (i = 0; i < 4; ++i) {
			 cubelet[64 ^ data[20 + cur_phase * 8 + i]].twist =
			(cubelet[64 ^ data[20 + cur_phase * 8 + i]].twist + 2 - i % 2) % 3,
			 cubelet[64 ^ data[20 + cur_phase * 8 + i + 4]].twist ^= cur_phase < 2;
		}
	}
	for (i = 0; i < 7; ++i) {
...
int do_search( int depth ) {
	int h = hashf();
	int i, k;
	int q = (cur_phase / 2 * 19 + 8) << 7;
	if ((depth < hash_table[cur_phase + 0][h % q] | depth < hash_table[cur_phase + 4][h / q]) ^ search_mode) {
		if (search_mode) {
			if (depth <= depth_to_go[h])
				return !h;
			else
				depth_to_go[h] = depth;
		}
		hash_table[cur_phase + 0][h % q] = std::min(hash_table[cur_phase + 0][h % q], static_cast<char>(depth));
		hash_table[cur_phase + 4][h / q] = std::min(hash_table[cur_phase + 4][h / q], static_cast<char>(depth));
		...
		for (k = 0; k < 6; ++k) {
			for (i = 0; i < 4; ++i) {
				...
				if (k < cur_phase * 2 & i != 1 || i > 2)
					continue;
				history_moves[history_index + 0] = k;
				history_rotpt[history_index + 0] = i;
				++history_index;
				if (do_search(depth - search_mode * 2 + 1))
					return 1;
				--history_index;
			}
		}
...
int main( int, char **argv ) {
	...
	for (cur_phase = 0; cur_phase < 4; ++cur_phase) {
		do_search(0);
	}
	for (i = 0; i < 20; ++i) {
		std::string str = argv[i + 1] + std::string("!");
	...
	for (k = 0; k < history_index; ++k) {
		std::cout << "FBRLUD"[history_moves[k] + 0] << history_rotpt[k] + 1 << " ";
	}
	return 0;
}
